...
public abstract class Personaje {
    ...
    protected Arma arma;
    protected ArrayList <Armadura> armadura;
    protected ArrayList <Artefacto> artefacto;
 ...
 */
    public String luchar(){
        return "El " + devuelveClase() + " " + getNombre() + " ataca con " + getDanoTotal() + " puntos de dano.\n";
    }
 ...
 */
    public String defender(){
        return getNombre() + " tiene " + getVida() + " puntos de Vida, " + getBlindajeTotal() + " puntos de Armadura y " + getResistenciaMagicaTotal() + " puntos de Resistencia Magica\n";
    }
 ...
 */
    ...
    public void setEArma(Arma a){
        Arma arma = new Arma(a);
    }
    
    public Arma getEArma(){
        return arma;
    }
    
    public void setEArmadura(ArrayList <Armadura> a){
        if(a.size()<=6){
            armadura = new ArrayList<>(a);
        }
    }
    
    public ArrayList<Armadura> getEArmadura(){
        return armadura;
    }
    
    public void setEArtefacto(ArrayList<Artefacto> a){
        if(a.size()<=3){
            artefacto = new ArrayList<>(a);
        }
    }
    
    public ArrayList<Artefacto> getEArtefacto(){
        return artefacto;
    }
    
    public void setEArtefacto(Artefacto a){
        if(artefacto.size()<3){
            artefacto.add(a);
        }
    }
    ...
    public int getDanoTotal(){
        return getAtaque() + arma.estadistica.get(0);
    }
    
    public int getAgilidadTotal(){
        return getAgilidad() + arma.estadistica.get(1);
    }
    
    public int getBlindajeTotal(){
        int contadorArmadura=0;
        
        for(int i=0; i<armadura.size(); i++){
            contadorArmadura+=armadura.get(i).estadistica.get(0);
        }
        
        return getArmadura() + contadorArmadura;
                
    }
    
    public int getResistenciaMagicaTotal(){
        int contadorArmadura=0;
        
        for(int i=0; i<armadura.size(); i++){
            contadorArmadura+=armadura.get(i).estadistica.get(1);
        }
        
        return getResistenciaMagica() + contadorArmadura;
    }
